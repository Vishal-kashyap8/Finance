...
data "azurerm_sql_managed_instance" "existing_instance" {
...
output "managed_instance_id" {
  ...
  value       = data.azurerm_sql_managed_instance.existing_instance.id
}

output "managed_instance_fqdn" {
  description = "Fully Qualified Domain Name of Managed Instance"
  value       = data.azurerm_sql_managed_instance.existing_instance.fqdn
}

output "connection_string" {
  description = "Connection string for the managed instance"
  value       = "Server=tcp:${data.azurerm_sql_managed_instance.existing_instance.fqdn},3342;Initial Catalog=<your_database>;User Id=<your_username>;Password=<your_password>;"
  sensitive   = true
}
